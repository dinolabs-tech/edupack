...
class PDF_Rotate extends FPDF
{
    ...
    public function getBottomMargin() { return $this->bMargin; }
...
if ($current_term_id === null) {
    customErrorHandler(E_ERROR, "Could not retrieve current term. Please configure it in the database.", __FILE__, __LINE__);
}

// 2) Fetch current session
$current_session_id = null;
$sqlSess = "SELECT csession FROM currentsession LIMIT 1";
$stmt = $conn->prepare($sqlSess);
$stmt->execute();
$res = $stmt->get_result();
if ($row = $res->fetch_assoc()) {
...
if ($current_session_id === null) {
    customErrorHandler(E_ERROR, "Could not retrieve current session. Please configure it in the database.", __FILE__, __LINE__);
}

// 3) Fetch classes and arms for filter dropdowns
$classes = [];
$sql = "SELECT class FROM class ORDER BY class";
...
$arms = [];
$sql = "SELECT arm FROM arm ORDER BY arm";
$stmt = $conn->prepare($sql);
$stmt->execute();
$res = $stmt->get_result();
while ($r = $res->fetch_assoc()) {
...
function generateDetailedAttendancePDF($records, $class, $arm) {
    ...
    $pdf->Cell(0, 10, "Demo School", 0, 1, 'C');
    $pdf->SetFont('Arial', '', 12);
    $pdf->Cell(0, 6, "123 Main Street, Anytown", 0, 1, 'C');
    $pdf->Cell(0, 6, "Contact: demo@school.com", 0, 1, 'C');
    $pdf->Ln(5);
...
<div class="container mt-4">
  ...
  <button class="btn btn-secondary" onclick="window.location.href='?class=<?= $selected_class ?>&arm=<?= $selected_arm ?>&download=1'">
    Print Detailed Sheet
  </button>

<?php if ($selected_class && $selected_arm): ?>
    <h2>Attendance Records for <?php echo $selected_class; ?> - <?php echo $selected_arm; ?></h2>
