 ...
 * chatbot.php
 *
 * This file implements a student-facing chatbot that provides academic performance analysis.
 * It retrieves the logged-in student's scores from the 'mastersheet' table for the current term
 * and generates a personalized response including overall average, suggestions for low-scoring subjects,
 * and commendations for high-scoring subjects.
 *
 * Key functionalities include:
 * - Database connection.
 * - Session management to identify the logged-in student.
...
// Start or resume a session. This is crucial for accessing the logged-in user's ID.
session_start();
// Retrieve the logged-in student's ID from the session, defaulting to an empty string if not set.
...
// Main chatbot logic: retrieves data from the 'mastersheet' table and analyzes student performance.
// This block executes only if a valid student ID is provided in the session.
if (!empty($student_id)) {
    ...
    if (!$result) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Calculate total score if it's not already available or is empty.
            if (empty($total)) {
                $total = $ca1 + $ca2 + $exam;
            }
        ...
        // Add improvement suggestions for low-scoring subjects.
        if (!empty($low_scores)) {
            $subjects_string = implode(", ", array_keys($low_scores)); // Join subject names for the message.
            $random_template = $suggestions[array_rand($suggestions)];     // Pick a random suggestion template.
            $response .= sprintf($random_template, $subjects_string) . " ";   // Format the template with subject names.
        } else {
        ...
        if (!empty($high_scores)) {
            $high_subjects = implode(", ", array_keys($high_scores)); // Join subject names for the message.
            $commend_template = $commendations[array_rand($commendations)]; // Pick a random commendation template.
            $response .= sprintf($commend_template, $high_subjects);       // Format the template with subject names.
        }
    } else {
    ...
    // If no student ID is provided in the session, prompt the user.
