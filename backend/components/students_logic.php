...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    $narration = htmlspecialchars($_POST['narration']);
    ...
    if (move_uploaded_file($_FILES["file_upload"]["tmp_name"], $file_path)) {
        ...
        $sql = "INSERT INTO prebursary (id, name, class, arm, term, gender, session, depositor, mobile, amount, narration, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssssssdss", $student_id, $name, $class, $arm, $term, $gender, $session, $depositor_name, $depositor_mobile, $amount_deposited, $narration, $file);
