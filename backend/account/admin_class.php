...
Class Action {
	...
	function login(){
		...
		$password = md5($password); // Note: MD5 is insecure, should be updated
	...
	function login2(){
		...
		$password = md5($password); // Note: MD5 is insecure, should be updated
		$qry = $this->db->prepare("SELECT * FROM complainants WHERE email = ? and password = ?");
		if ($qry === false) {
			customErrorHandler(E_ERROR, "Error preparing statement: " . $this->db->error, __FILE__, __LINE__);
			return 3;
		}
	...
	function save_fees(){
		...
		if(empty($id)){
			...
			$get_fee = $this->db->query("SELECT total_fee FROM student_ef_list WHERE id = $id")->fetch_array();
			$total_fee = str_replace(',', '', $total_fee) + $get_fee['total_fee'];
