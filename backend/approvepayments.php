...
<style>
  ...
  #studentImage {
    ...
    max-height: 300px;
    display: none;
    /* Hide by default; shown when a student is selected */
  }
</style>
...
<body>
                          ...
                          <?php while ($row = $students->fetch_assoc()): ?>
                            <tr>
                              ...
                              <td class="d-flex">
                                ...
                                <a href="?date=<?= htmlspecialchars($row['date']) ?>" class="btn btn-warning btn-icon btn-round ps-1 me-2"><span class="btn-label">
                                    <i class="fa fa-edit"></i></a>
                                <!-- Delete button: deletes the record from prebursary table -->
                                <a href="?delete_id=<?= htmlspecialchars($row['date']) ?>" class="btn btn-danger btn-icon btn-round ps-1" onclick="return confirm('Are you sure you want to delete this student?')"><span class="btn-label">
  ...
  <script>
    ...
    studentImage.addEventListener('click', () => {
      ...
      } else if (studentImage.webkitRequestFullscreen) {
        /* Safari */
        studentImage.webkitRequestFullscreen();
      } else if (studentImage.msRequestFullscreen) {
        /* IE11 */
  ...
  <script>
    ...
    function refreshTable() {
      $.ajax({
        ...
        data: {
          action: 'fetch_table'
        }, // Specify action to fetch updated table rows.
        success: function(response) {
      ...
      if (studentId.length > 0) {
        $.ajax({
          ...
          data: {
            date: studentId,
            action: 'fetch'
          }, // Send deposit date and fetch action.
          success: function(response) {
