...
$sql = "SELECT * FROM mastersheet WHERE id = ? ORDER BY csession, term, subject";
...
$stmt = $conn->prepare($sql);
if ($stmt === false) {
    customErrorHandler(E_ERROR, "Error preparing calendar events statement: " . $conn->error, __FILE__, __LINE__);
} else {
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result) {
        ...
        customErrorHandler(E_ERROR, "Failed to fetch calendar events: " . $conn->error, __FILE__, __LINE__);
    }
    $stmt->close();
}
      ...
      <div class="main-panel">
        ...
        <div class="container">
          <div class="page-inner">
            ...
            <h6 class="h6">School Address and contact</h6>
