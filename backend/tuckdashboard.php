...
function executeQuery($conn, $sql) {
    ...
    // No parameters to bind in these queries
    if ($stmt->execute() === false) {
        customErrorHandler(E_ERROR, "Error executing statement: " . $stmt->error, __FILE__, __LINE__);
        return null;
    }
    $result = $stmt->get_result();
    if ($result === false) {
        customErrorHandler(E_ERROR, "Error getting result: " . $stmt->error, __FILE__, __LINE__);
        return 0;
    }
    $stmt->close();
    return $result;
...
// Execute the queries
$total_sales_result = executeQuery($conn, $total_sales_query);
$total_profit_result = executeQuery($conn, $total_profit_query);
$inventory_qty_result = executeQuery($conn, $inventory_qty_query);
$inventory_sum_result = executeQuery($conn, $inventory_sum_query);
$total_transactions_result = executeQuery($conn, $total_transactions_query);
$out_of_stock_result = executeQuery($conn, $out_of_stock_query);
$tuck_student_result = executeQuery($conn, $tuck_students_qty_query);
$tuck_balance_result = executeQuery($conn, $tuck_balance_query);
$tuck_low_balance_result = executeQuery($conn, $tuck_low_balance_query);

// Fetch the results
$total_sales = $total_sales_result ? $total_sales_result->fetch_row()[0] : 0;
$total_profit = $total_profit_result ? $total_profit_result->fetch_row()[0] : 0;
$inventory_qty = $inventory_qty_result ? $inventory_qty_result->fetch_row()[0] : 0;
$inventory_sum = $inventory_sum_result ? $inventory_sum_result->fetch_row()[0] : 0;
$total_transactions = $total_transactions_result ? $total_transactions_result->fetch_row()[0] : 0;
$out_of_stock = $out_of_stock_result ? $out_of_stock_result->num_rows : 0;
$total_tuck_students = $tuck_student_result ? $tuck_student_result->fetch_row()[0] : 0;
$total_students_balance = $tuck_balance_result ? $tuck_balance_result->fetch_row()[0] : 0;
$total_low_balance = $tuck_low_balance_result ? $tuck_low_balance_result->fetch_row()[0] : 0;
