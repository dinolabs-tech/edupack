...
$term_sql = "SELECT cterm FROM currentterm WHERE id=1";
...
$session_sql = "SELECT csession FROM currentsession WHERE id=1";
    ...
    if (isset($_POST['bulk_submit']) && isset($_FILES["csv_file"])) {
      ...
      if (($handle = fopen($_FILES["csv_file"]["tmp_name"], "r")) !== FALSE) {
        ...
        $calculaterankQuery = "
           ...
           ON m.subject = ranks.subject AND m.class = ranks.class AND m.term = ranks.term AND m.name = ranks.name
           SET m.position = ranks.position;
         ";

        $rank_stmt = $conn->prepare($calculaterankQuery);
        if ($rank_stmt === false) {
            customErrorHandler(E_ERROR, "Error preparing rank update statement: " . $conn->error, __FILE__, __LINE__);
        } else {
            if ($rank_stmt->execute()) {
                // $class_messages[] = "Positions updated successfully.";
            } else {
                $class_messages[] = "Error updating positions: " . $rank_stmt->error;
            }
            $rank_stmt->close();
        }
      }
...
function fetch_unique_values($conn, $column, $table)
{
  ...
  if ($result === false) {
    customErrorHandler(E_ERROR, "Error fetching unique values: " . $conn->error, __FILE__, __LINE__);
    return [];
  }
...
$classes = fetch_unique_values($conn, 'class', 'class');
$arms = fetch_unique_values($conn, 'arm', 'arm');
$subjects = fetch_unique_values($conn, 'subject', 'subject');
