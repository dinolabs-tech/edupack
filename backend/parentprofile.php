...
if (isset($_POST['update_password'])) {
  ...
  if ($new_password !== $confirm_password) {
    ...
    $user_id = $_SESSION['user_id'];
    $stmt = $conn->prepare("UPDATE parent SET password=? WHERE id=?");
...
if (isset($_POST['update_profile'])) {
  ...
  $user_id = $_SESSION['user_id'];
  $stmt = $conn->prepare("UPDATE parent SET mobile=?, email=? WHERE id=?");
  $stmt->bind_param("sss", $mobile, $email, $user_id);
...
$student_details = [];
$sql = "SELECT mobile, email FROM parent WHERE id = ?";
if ($stmt = $conn->prepare($sql)) {
  $stmt->bind_param("s", $user_id);
  $stmt->execute();
  $stmt->bind_result($student_details['mobile'], $student_details['email']);
  $stmt->fetch();
  $stmt->close();
} else {
...
// Fetch the logged-in Parent's name
$user_id = $_SESSION['user_id'];
                  ...
                  <form action="" method="post">
                    ...
                    <div class="form-group">
                      <input type="text" class="form-control" id="mobile" name="mobile" placeholder="Enter Mobile"
                        value="<?php echo htmlspecialchars($student_details['mobile']); ?>" />
                      <!-- <small id="emailHelp2" class="form-text text-muted">Your Mobile will only be shared with your
                        class Peers.</small> -->
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <!-- <small id="emailHelp2" class="form-text text-muted">Your E-mail will only be shared with your
                        class Peers.</small> -->
                    </div>
